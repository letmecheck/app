class King < Piece
  def valid_move?(destination_x, destination_y)
    ...
    (-1..1).cover?(x_offset) && (-1..1).cover?(y_offset) || valid_castling?(destination_x, destination_y)
  end
  ...
  def valid_castling?(destination_x, destination_y)
  ...
  def rook_requirements_met?(destination_x, destination_y)
    ...
    !obstructed?(rook_x_coord, destination_y)
  end
  ...
  def king_traversal_under_attack?(destination_x, destination_y)
    # The argument for destination_x will be either 3 or 7 if castling is attempted.
    king_x_array = (destination_x == 3) ? [3, 4, 5] : [5, 6, 7]
    ...
    king_x_array.any? { |x_coord| game.square_threatened_by?(opponent_color, x_coord, destination_y) }
  end
end
