class Pawn < Piece
  def valid_move?(destination_x, destination_y)
    offset = movement_by_axis(destination_x, destination_y)

    offset[1] *= -1 if color == 'black'

    return valid_standard_move?(destination_x, destination_y) if offset == [0, 1]

    return valid_double_move?(destination_x, destination_y) if offset == [0, 2]

    return valid_capture?(destination_x, destination_y) if [[-1, 1], [1, 1]].include? offset
  ...
  def valid_standard_move?(destination_x, destination_y)
    !game.piece_at(destination_x, destination_y)
  end

  def valid_double_move?(destination_x, destination_y)
    return false unless y_coord == nth_rank(2)
    return false if game.piece_at(destination_x, nth_rank(3))
    !game.piece_at(destination_x, destination_y)
  end
